...
/**
 * NOTE:  This is just a clone of the resource here:  https://github.com/aionick/OfflineTransactionSigner/blob/master/src/main/crypto/Blake2b.java
 */
    ...
    class Param implements AlgorithmParameterSpec {
        ...
        public final Param setDigestLength(int len) {
            ...
            assert len <= Spec.MAX_DIGEST
                : Engine.Assert.assertFail("len", len, Engine.Assert.inclusiveUpperBound, Spec.MAX_DIGEST);
        ...
        public final Param setKey(final byte[] key) {
            ...
            assert key.length <= Spec.MAX_KEY
                : Engine.Assert.assertFail("key.length", key.length, Engine.Assert.inclusiveUpperBound, Spec.MAX_KEY);
        ...
        public final Param setLeafLength(int leaf_length) {
            assert leaf_length >= 0
                : Engine.Assert.assertFail("leaf_length", leaf_length, Engine.Assert.inclusiveLowerBound, 0);
        ...
        public final Param setNodeOffset(long node_offset) {
            assert node_offset >= 0
                : Engine.Assert.assertFail("node_offset", node_offset, Engine.Assert.inclusiveLowerBound, 0);
        ...
        public final Param setInnerLength(int inner_length) {
            assert inner_length >= 0
                : Engine.Assert.assertFail("inner_length", inner_length, Engine.Assert.inclusiveLowerBound, 0);
        ...
        public final Param setSalt(final byte[] salt) {
            ...
            assert salt.length <= Spec.MAX_SALT
                : Engine.Assert.assertFail("salt.length", salt.length, Engine.Assert.inclusiveUpperBound, Spec.MAX_SALT);
